...
select cron.schedule('tapmap_events_tm',    '23 */6 * * *',   $$select private.invoke_job('events_sync', '{"adapters":["ticketmaster"]}')$$);
select cron.schedule('tapmap_events_wwoz',  '41 */6 * * *',   $$select private.invoke_job('events_sync', '{"adapters":["wwoz"],"days":2}')$$);
